...
import { rpx2px } from '../../uni-core/helpers/util';
import {
  createSvgIconVNode,
  ICON_PATH_BACK,
} from '../../uni-core/helpers/icon';
...
function useLayout(
  ...
  function normalizeCurrentValue(current: number) {
    const { length } = swiperContexts.value;
    if (!length) {
      return -1;
    }
  ...
  function animateFrameFuncProto() {
    ...
    const { toPos } = innerAnimating;
    const { acc } = innerAnimating;
    const { endTime } = innerAnimating;
    const { source } = innerAnimating;
...
const useSwiperNavigation = /* #__PURE__*/ (
  ...
  function createNavigationTsx() {
    const navigationClass = {
      ...
      'swiper-next__navigation-vertical': props.vertical,
    };
