...
<template>
  <div class="icons-box">
    <div
      ...
      :key="index"
      class="icons-item"
      @click="onClipboard(item)"
    >
      <span
        class="icons-tip"
        :class="{ show: item.tipShow }"
      >复制成功</span>
...
export default {
  ...
  methods: {
    onClipboard(data) {
      const index  = this.iconsList.findIndex(item => item.tipShow);
      if (index !== -1) {
        this.iconsList[index].tipShow = false;
      }
      const _this = this;
      const clipboard = new Clipboard('.icons-item', {
        text() {
          return data.text;
        },
      });
