...
`press-popup` 与 `press-popup-plus` 展示逻辑有很大不同，`press-popup` 为受控组件，`show` 默认为`false`。外界传 `true` 时，展示进入动画，外界传 `false` 时，展示退出动画。
...
`press-popup-plus` 由于历史原因，`isShow` 属性（对应 `press-popup` 的 `show`）默认为 `true`，且点击 `cancel`、`confirm` 时会关闭 `popup`，大量业务这样使用：
...
`press-popup-plus` 为了防止滚动穿透，在外层以及蒙层使用了 `@touch.stop="noop"`。如果内部元素需要滚动，可以使用 `scroll-view`，单纯设置元素 `overflow: scroll` 可能会在小程序下无效。可以参考上面“函数式调用”示例部分。
