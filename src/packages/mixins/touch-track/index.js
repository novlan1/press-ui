...
export default {
  ...
  methods: {
    touchTrack(element, method, useCancel) {
      const self = this;
      let x0 = 0;
      let y0 = 0;
      let x1 = 0;
      ...
      let hasTouchStart;
      let hasMouseDown;
      addListenerToElement(element, 'touchstart', ($event) => {
        hasTouchStart = true;
      ...
      addListenerToElement(element, 'mousedown', ($event) => {
        hasMouseDown = true;
      ...
      addListenerToElement(element, 'touchmove', ($event) => {
        if ($event.touches.length === 1 && $eventOld) {
      ...
      this.__clickEventListener  = function ($event) {
        $event.preventDefault();
        $event.stopPropagation();
      };
      const clickEventListener = this.__clickEventListener;

      this.__mouseMoveEventListener = function ($event) {
      ...
      const mouseMoveEventListener = this.__mouseMoveEventListener;

      document.addEventListener('mousemove', mouseMoveEventListener);
      addListenerToElement(element, 'touchend', ($event) => {
      ...
      this.__mouseUpEventListener = ($event) => {
        hasMouseDown = false;
        if (!hasTouchStart && $eventOld) {
      ...
      const mouseUpEventListener = this.__mouseUpEventListener;

      document.addEventListener('mouseup', mouseUpEventListener);

      addListenerToElement(element, 'touchcancel', ($event) => {
        if ($eventOld) {
          hasTouchStart = false;
